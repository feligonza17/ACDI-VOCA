---
title: 'Analisis Diferencias Olas PAR'
...
El objetivo de este documento es empezar a cruzar variables de diferentes bases de datos. En efecto, las bases anteriores (y sus analisis) podian estar presentando problemas de multicolinealidad. Las variables de Gobierno, a pesar de ser diferentes eran parecidas. Por lo tanto, aqui se hara alguna estadistica inferencial mezclando bases de datos para poder ver que puede predecir el indice de confianza en los municipios PAR. 
...
Tenga en cuenta que **solo un codigo parcial de esta seccion se va a mostrar** Esto sencillamente para no sobrecargar el documento. Si usted esta interesado en ver el codigo ir a la siguiente pagina:  [GitHub](https://github.com/feligonza17/ACDI-VOCA) y buscar el archivo con el nombre: *reg_olas_PAR.Rmd*


##Diferencia de resultados entre Ola 2017 y Ola 2019

Aqui se va a crear la variable que mide la diferencia en el puntaje en los indices de confianza (instiucion, medios y vecinos) a nivel municipal.

La variable se calcula de la siguiente forma: $\bar{x}_{indice2019} - \bar{x}_{indice2017}$



```{r}
base_stata_2019$dif_confianza_instituciones <-  base_stata_2019$confianza_instituciones_men2019- base_stata_2019$confianza_instituciones_arr_men

base_stata_2019$dif_confianza_medios <-  base_stata_2019$confianza_medios_mean2019 - base_stata_2019$confianza_medios_arr_mean

base_stata_2019$dif_confianza_vec <-  base_stata_2019$confianza_vec_mean2019 - base_stata_2019$confianza_vec_arr_mean
...
descrip_alumnos_cienmil <-descrip_alumnos_cienmil%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)


descrip_t_establ <- summaryBy(t_establ ~ municipio+codmpio, cede_educacion_44, FUN=c(sum,mean,sd), na.rm=T )	#Codigo para hacer descriptivos de los homicidios (collapse por anos)

descrip_t_establ <- descrip_t_establ %>%filter(codmpio%in%mpios_par)%>% arrange(municipio)

print(cbind(base_stata_2019[1],descrip_t_establ))
...
descrip_t_establ_5antes <- cede_educacion_44 %>% 
  filter(codmpio%in%mpios_par)%>%
  filter(ano<"2000-04-27") #Codigo para seleccionar solamente las variables que sean menores al ano 2008
descrip_t_establ_5antes <- summaryBy(t_establ ~ municipio+codmpio, descrip_t_establ_5antes, FUN=c(sum,mean,sd), na.rm=T )	#Descriptivos de esos anos. 
...
descrip_t_establ_5despues <- summaryBy(t_establ ~ municipio+codmpio, descrip_t_establ_5despues, FUN=c(sum,mean,sd), na.rm=T )	#Descriptivos de esos anos. 
```
...
print(cbind(base_stata_2019[1],diff_alumnos_cienmil_mean))
...
print(cbind(base_stata_2019[1],diff_t_establ_mean))
```
...
descrip_phds_cienmil <- descrip_phds_cienmil%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_phds_cienmil_mean))
```
...
descrip_estudiantes_proy_cienmil <- descrip_estudiantes_proy_cienmil%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_estudiantes_proy_cienmil_mean))
```
...
descrip_desemp_fisc <- descrip_desemp_fisc%>%filter(codmpio%in%mpios_par)%>% arrange(municipio) 
...
print(cbind(base_stata_2019[1],diff_desemp_fisc_mean))
```
...
descrip_desemp_int <- descrip_desemp_int%>%filter(codmpio%in%mpios_par)%>% arrange(municipio) 
...
print(cbind(base_stata_2019[1],diff_desemp_int_mean))
...
descrip_inv_total_cienmil <- descrip_inv_total_cienmil%>%filter(codmpio%in%mpios_par)%>% arrange(municipio) 
...
print(cbind(base_stata_2019[1],diff_inv_total_cienmil_mean))
```
...
descrip_inv_fortinst_cienmil <- descrip_inv_fortinst_cienmil%>%filter(codmpio%in%mpios_par)%>% arrange(municipio) 
...
print(cbind(base_stata_2019[1],diff_inv_fortinst_cienmil_mean))
```
...
descrip_IGA_total <- descrip_IGA_total%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_IGA_total_mean))
```
...
descrip_TMI <- descrip_TMI%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_TMI_mean))
```
...
descrip_nbi <- descrip_nbi%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_nbi_mean))
print(cbind(base_stata_2019[1],diff_nbi_sd))
```
...
descrip_prestadores_cienmil <- descrip_prestadores_cienmil%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_prestadores_cienmil_mean))
```
...
descrip_subsidiado_cienmil <- descrip_subsidiado_cienmil%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_subsidiado_cienmil_mean))
```
...
descrip_contributivo_cienmil <- descrip_contributivo_cienmil%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_contributivo_cienmil_mean))
```
...
descrip_nuf_peq <- descrip_nuf_peq%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_nuf_peq_mean))
```
...
descrip_nuf_medianos <- descrip_nuf_medianos%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_nuf_medianos_mean))
```
...
descrip_nuf_grandes <- descrip_nuf_grandes%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_nuf_grandes_mean))
```
...
descrip_vrf_peq <- descrip_vrf_peq%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_vrf_peq_mean))
```
...
descrip_vrf_medianos <- descrip_vrf_medianos%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_vrf_medianos_mean))
```
...
descrip_vrf_grandes <- descrip_vrf_grandes%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_vrf_grandes_mean))
```
...
descrip_ba_tot_nu <- descrip_ba_tot_nu%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_ba_tot_nu_mean))
```
...
descrip_ba_tot_vr <- descrip_ba_tot_vr%>%filter(codmpio%in%mpios_par)%>% arrange(municipio)
...
print(cbind(base_stata_2019[1],diff_ba_tot_vr_mean))
```
...
descrip_homi <- descrip_homi%>%filter(codmpio%in%mpios_par)%>% arrange(Municipio) 
...
print(cbind(base_stata_2019[1],diff_homi_mean))
...
descrip_ataques <- descrip_ataques%>%filter(codmpio%in%mpios_par)%>% arrange(Municipio) 
...
print(cbind(base_stata_2019[1],diff_ataques_mean))
...
descrip_secuestro <- descrip_secuestro%>%filter(codmpio%in%mpios_par)%>% arrange(Municipio) 
...
print(cbind(base_stata_2019[1],diff_secuestro_mean))
```
...
descrip_secuestrop <- descrip_secuestrop%>%filter(codmpio%in%mpios_par)%>% arrange(Municipio) 
...
print(cbind(base_stata_2019[1],diff_secuestrop_mean))
```
...
descrip_desmovilizados <- descrip_desmovilizados %>%filter(codmpio%in%mpios_par)%>% arrange(Municipio) 
...
print(cbind(base_stata_2019[1],diff_desmovilizados_mean))
```
...
agregados_regresiones <- cbind(descrip_homi$homi_cienmil.mean,
                      ...
                      base_stata_2019$dif_confianza_instituciones,
                      base_stata_2019$dif_confianza_medios,
                      base_stata_2019$dif_confianza_vec)
...
```{r}
regFinalAgregados_PAR <- lm(dif_confianza_insti ~ total_establecimientos + doctores_cienmil + `Inv Total` +  `Gobierno Abierto` + Ataques_Pobl_Civil + Homicidios, data = agregados_regresiones)
summary(regFinalAgregados_PAR)  


regFinalAgregados_PAR <- lm(dif_confianza_insti ~ numero_pequenos+total_establecimientos +  NBI + Ataques_Pobl_Civil+estudiantes_edad_estudiar+Secuestros+ Homicidios  , data =agregados_regresiones)
summary(regFinalAgregados_PAR)  


# Hacemos la regresion final 

regFinalAgregados_PAR <- lm(dif_confianza_medios ~ total_establecimientos + doctores_cienmil + `Inv Total` +  `Gobierno Abierto` + Ataques_Pobl_Civil + Homicidios, data = agregados_regresiones)
summary(regFinalAgregados_PAR)  

regFinalAgregados_PAR <- lm(dif_confianza_medios ~ numero_pequenos+total_establecimientos +  NBI + Ataques_Pobl_Civil+estudiantes_edad_estudiar+Secuestros+ Homicidios  , data =agregados_regresiones)
summary(regFinalAgregados_PAR)  

# Hacemos la regresion final 

regFinalAgregados_PAR <- lm(dif_confianza_vec ~ total_establecimientos + doctores_cienmil + `Inv Total` +  `Gobierno Abierto` + Ataques_Pobl_Civil + Homicidios, data = agregados_regresiones)
summary(regFinalAgregados_PAR)  

regFinalAgregados_PAR <- lm(dif_confianza_vec ~ numero_pequenos+total_establecimientos +  NBI + Ataques_Pobl_Civil+estudiantes_edad_estudiar+Secuestros+ Homicidios  , data =agregados_regresiones)
summary(regFinalAgregados_PAR)  
...
Hacemos la regresion final 
```{r}
regFinaldeltas_PAR <- lm(dif_confianza_insti ~ total_establecimientos + doctores_cienmil + `Inv Total` +  `Gobierno Abierto` + Ataques_Pobl_Civil + Homicidios, data = deltas_regresiones)
summary(regFinaldeltas_PAR)  


regFinaldeltas_PAR <- lm(dif_confianza_insti ~ numero_pequenos+total_establecimientos +  NBI + Ataques_Pobl_Civil+estudiantes_edad_estudiar+Secuestros+ Homicidios, data = deltas_regresiones)
summary(regFinaldeltas_PAR)  


regFinaldeltas_PAR <- lm(dif_confianza_insti ~ `Gobierno Abierto`+total_establecimientos +  NBI + Ataques_Pobl_Civil+numero_medianos+Secuestros+ Homicidios+doctores_cienmil, data = deltas_regresiones)
summary(regFinaldeltas_PAR)  
```
